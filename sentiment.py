...
__TRAINING_SET = "datasets" + os.path.sep + "training.csv"
...
def __confusionMatrix(test_y, predict_y, classes):
    cm = confusion_matrix(test_y, predict_y)

    print("    ", classes)
    for x in range(0, len(cm) + 1):
        print(classes[x], cm[x])
...
def __createDatasetFromDtabase(num_pos, num_neg, num_neu, path):
    trainingDataSet = db.getTrainingDataSet(num_pos, num_neg, num_neu)
    print("Creando dataset:", path)
    ds.writeCSV(["polarity", "text"], path)
    for row in trainingDataSet.values:
        ds.writeCSV(row, path)
...
def initNeuralNet():
    ...
    if os.path.exists(__VOCABULARY) and os.path.exists(__CLASSES):
        ...
        if os.path.exists(__TRAINING_SET):     
            ...
            trainingDataSet = __createDatasetFromDtabase(num_pos, num_neg, num_neu, __TRAINING_SET)

        vocabulary = __getVocabulary(trainingDataSet["text"], 2)
...
if __name__ == '__main__':
    ...
    path = "datasets" + os.path.sep + "validation.csv"
